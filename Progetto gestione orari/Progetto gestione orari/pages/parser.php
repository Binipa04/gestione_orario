<?php
    ...
    try{
      ...
      $query= $pdo->prepare("SELECT * FROM prof");
      $query->execute();
      ...
      while(!feof($oreFile)) {
        ...
        if($i>0){
          ...
          if(sizeof($virgolette)>2 && !str_contains($virgolette[1],"<")){//Pezzone tattico
            ...
            $stmt= $pdo->prepare($sql); 
            $stmt->execute([$z, $place,$dati[1], $pino[7], explode("h", $pino[8])[0], $dati[4] , $class]);
            ...
            $sql = "INSERT INTO ora (id, luogo, durata, giorno, ora, idMateria, idClasse) VALUES (?,?,?,?,?,?,?)";
            $stmt= $pdo->prepare($sql); 
            $stmt->execute([$z, $place, $dati[1], $dati[13], $dati[14], $dati[4] , $class]);
            ...
            /*
            $sql = "INSERT INTO profinsegnaclasse (idProf, idClasse) VALUES (?,?)";
            $stmt= $pdo->prepare($sql); 
            $stmt->execute([array_search($dati[5],$professoriA,false),$z]);
            */
            $z++;
